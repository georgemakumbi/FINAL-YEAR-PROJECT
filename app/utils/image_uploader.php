...
/**
 * Uploads the system logo.
 * If Vercel Blob is configured (token is set), it uploads to Vercel Blob.
 * Otherwise, it falls back to local file upload.
 * 
 * @param array $file The $_FILES['system_logo'] array.
 * @param string|null &$error Outputs the error message if upload fails.
 * @return string|null Returns the logo path (relative local path or remote URL), or null on failure.
 */
function upload_system_logo($file, &$error = null) {
    if (!isset($file) || $file['error'] !== UPLOAD_ERR_OK) {
        $error = "No file uploaded or file upload error occurred.";
        return null;
    }
    ...
    $ext = $allowed[$mime];
    // Use a unique name to bypass browser caching immediately on upload
    $filename = 'logo_' . bin2hex(random_bytes(4)) . '.' . $ext;
    ...
    if ($token) {
        ...
        if (!file_exists($upload_dir)) {
            if (!mkdir($upload_dir, 0777, true)) {
                $error = "Failed to create local uploads directory.";
                return null;
            }
        }
